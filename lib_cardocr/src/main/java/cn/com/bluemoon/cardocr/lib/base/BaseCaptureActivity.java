...
public abstract class BaseCaptureActivity extends BasePermissionFragmentActivity
                ...
                mHandler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            else if(cartType == CardType.TYPE_TEXT)
                            {
                                isSuccessful = handleTextData(isSuccessful,bundle, responseBody);
                            }
    ...
    protected boolean handleIdCardData(boolean isSuccessful, Bundle bundle, String responseBody) {
        ...
        if (cartType == CardType.TYPE_ID_CARD_FRONT) {
            IDCardFrontBean iDCardBean =  GsonUtils.jsonToClass(responseBody, IDCardFrontBean.class);
            ...
            IDCardBackBean iDCardBean =   GsonUtils.jsonToClass(responseBody, IDCardBackBean.class);
            if (iDCardBean.errorcode == 0) {
    ...
    protected boolean handleBankCardData(boolean isSuccessful, Bundle bundle, String responseBody) {
        //银行卡
        BankCardBean bankCardBean =    GsonUtils.jsonToClass(responseBody, BankCardBean.class);
     ...
     * @return
     */
    protected boolean handleTextData(boolean isSuccessful, Bundle bundle, String responseBody) {
        //文本文字
          TextOcrItem textOcrItem  =    GsonUtils.jsonToClass(responseBody, TextOcrItem.class);
        if (textOcrItem.errorcode == 0) {
            isSuccessful = true;
            bundle.putSerializable(BUNDLE_DATA, textOcrItem);
        }
        return isSuccessful;
    }
    ...
    protected boolean handleDrivingLicenseData(boolean isSuccessful, Bundle bundle, String responseBody) {
        //行驶证、驾驶证
        DrivingLicenseBean drivinglicenseBean =   GsonUtils.jsonToClass(responseBody, DrivingLicenseBean.class);
            ...
            try {
                mCamera.takePicture(null, null, new Camera.PictureCallback() {
                    ...
                    public void onPictureTaken(final byte[] bytes, Camera camera) {
                        isControl = false;
                        CaptureActivity.sImageData = bytes;
